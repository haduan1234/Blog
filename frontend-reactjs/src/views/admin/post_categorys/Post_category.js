...
const Post_categorys = () => {
    ...
    const fetchPost_category = useCallback(async (search = undefined) => {
        try {
            let res = await getPost_category(search, currentpage)
            if (!!res.data) {
            ...
            <CCard className="m-1">
                ...
                <CCardBody xs={12} style={{ fontSize: 15 }}>
                    <CTable class="table" >
                        ...
                        <CTableBody>
                            {console.log("data:", post_categorys)}
